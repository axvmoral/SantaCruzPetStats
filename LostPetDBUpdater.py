...
def UpdateLostPetDB(pet: str, db: str, table) -> None:
    ...
    cur.executemany(
        f"INSERT INTO NewWebData VALUES{VALUES}", (row[1] for row in df.iloc[:, 1:].iterrows())
    )
    cur.execute(
        f"SELECT {table}.ID FROM {table} LEFT JOIN NewWebData ON {table}.ID = NewWebData.ID WHERE NewWebData.ID IS NULL AND {table}.LastDate IS NULL"
    )
    gone = cur.fetchall()
    print(f"{len(gone)} {pet.lower()}(s) gone from website")
    cur.executemany(f'UPDATE {table} SET LastDate = "{TODAY}" WHERE ID = ?', gone)
    cur.execute(
        f"SELECT NewWebData.* FROM NewWebData LEFT JOIN {table} ON NewWebData.ID = {table}.ID WHERE {table}.ID IS NULL"
    )
    new = cur.fetchall()
...
if __name__ == "__main__":
    ...
    UpdateLostPetDB(
        pet="CAT",
        db=r"C:\Users\axvmo\OneDrive\Documents\GitHub\SantaCruzPetStats\data\Pets.db",
        table="LostCats",
    )
    UpdateLostPetDB(
        pet="DOG",
