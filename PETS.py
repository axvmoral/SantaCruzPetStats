...
class PETS:
        ...
        for ROW in ROWS[1:]:
            for j, ENTRY in enumerate(ROW.find_all("td")):
                if j == 0:
                    ...
                    data["Image"].append(URL_ROOT + ENTRY.img.get("src"))
    ...
    def UpdateData(self, DATA_PATH: str) -> None:
        ...
        if EXISTS:
            ...
            NEW_DATA = self.PrepareData(self.DATAFRAME[DIFFERENCE_INDEX])
        else:
            DIRNAME = os.path.dirname(DATA_PATH)
            if not os.path.exists(DIRNAME):
                os.mkdir(DIRNAME)
            NEW_DATA = self.PrepareData(self.DATAFRAME)
        NEW_DATA.reset_index().to_csv(DATA_PATH, mode="a", header= not EXISTS, index=False)
        return None
