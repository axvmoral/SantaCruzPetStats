...
def group_det(text: str, degree: int) -> bool:
    if not text:
        ...
        for word in [
            word.strip(".?!:; ").lower()
            for word in text.translate(text.maketrans(".!?:;", "     ")).split()
        ]:
...
def visit_links(links: list) -> list[list]:
    ...
    for link in links:
        ...
        date = re.findall(
            "I have been at the shelter since (.+?)[.]",
            soup.find(attrs={"class": "DetailDesc"}).text,
        )[0]
        EnterDate.append(datetime.strptime(date, r"%b %d, %Y").strftime("%Y-%m-%d"))
...
def get_treatment(gender: str) -> int:
    if gender.startswith("U"):
        ...
        gender.startswith("M")
        or gender.startswith("F")
        and gender.endswith("SPAYED")
        or gender.endswith("NEUTERED")
    ):
        return 1
    else:
        return 0
